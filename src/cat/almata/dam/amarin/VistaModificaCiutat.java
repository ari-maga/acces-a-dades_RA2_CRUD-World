...
public class VistaModificaCiutat extends JDialog {

	private static final long serialVersionUID = 1L;
	private final JPanel pnlContingut = new JPanel();
	private JTextField txfNom;
	private JTextField txfPoblacio;
	private VistaModificaCiutat editaCiutat;
	private JComboBox<String> cbxPais_1;
	private JComboBox<String> cbxDistricte;
	private DTOMainformCiutat ciutat;

	
	/**
	 * Create the dialog.
	 */
	public VistaModificaCiutat(JFrame owner, DTOMainformCiutat ciutatOriginal) {
		super(owner);
		editaCiutat = this;
		ciutat = ciutatOriginal;
		...
		{
			JLabel lblTitol = new JLabel("Edita les dades de la ciutat");
				...
				cbxPais_1.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						try {
							...
							JOptionPane.showMessageDialog(editaCiutat,  "ERROR! En llistar els Districtes (VistaModificaCiutat, lin. 1)\n\n"+e1.getMessage(),"Error de SQL", JOptionPane.ERROR_MESSAGE);
						}
					}
				});
			} catch (SQLException e) {
				JOptionPane.showMessageDialog(editaCiutat,  "ERROR! En llistar els Paisos (VistaModificaCiutat, lin. 107)\n\n"+e.getMessage(),"Error de SQL", JOptionPane.ERROR_MESSAGE);
			}
		...
		}
		{
			...
			{
				JButton btnAfegir = new JButton("Modifica");
				btnAfegir.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if( comprovacioPoblacio()&&comprovacioCiutatRepetida()) {
							//Actualitzar la ciutat
							try {
								DataActions.updateCiutat(new DTOMainformCiutat(
										ciutat.getId(),
										txfNom.getText(),
								...
								JOptionPane.showMessageDialog(editaCiutat,  "ERROR! En actualitzar laciutat (VistaModificaCiutat, lin. 219)\n\n"+e1.getMessage(),"Error de SQL", JOptionPane.ERROR_MESSAGE);
							}
					...
					private boolean comprovacioCiutatRepetida() {
						DTOMainformCiutat ciutatNova = new DTOMainformCiutat(
								ciutatOriginal.getId(),
								txfNom.getText(),
								(String) cbxPais_1.getSelectedItem(),
								(String) cbxDistricte.getSelectedItem(),
								Long.parseLong(txfPoblacio.getText())
						...
						try {
							if(!DataActions.esCiutatRepetida(ciutatNova)) {
								return true;
							}else {
								JOptionPane.showMessageDialog(editaCiutat,  "ERROR! Ja hi ha una ciutat en aquest país amb aquest nom!","Nom no vàlid", JOptionPane.WARNING_MESSAGE);
							}
						} catch (HeadlessException e) {
							JOptionPane.showMessageDialog(editaCiutat,  "ERROR! Excepió no prevista (VistaModificaCiutat, lin. 233)\n\n"+e.getMessage(),"Error de Headless Exception", JOptionPane.ERROR_MESSAGE);
							return false;
						} catch (SQLException e) {
							JOptionPane.showMessageDialog(editaCiutat,  "ERROR! En buscar si la ciutat esta repeticda en base de dades (VistaModiciaCiutat, lin. 233)\n\n"+e.getMessage(),"Error de SQL", JOptionPane.ERROR_MESSAGE);
							return false;
						}
						return false;
		...
		posaContingutsCiutat();
	}


	private void posaContingutsCiutat() {
		txfNom.setText(ciutat.getNom());
		cbxPais_1.setSelectedItem(ciutat.getPais());
		cbxDistricte.setSelectedItem(ciutat.getDistricte());
		txfPoblacio.setText(String.valueOf(ciutat.getPoblacio()));
